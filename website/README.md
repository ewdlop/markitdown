...
若您已在本機安裝 [`markitdown-mcp`](https://pypi.org/project/markitdown-mcp/) 並啟動 HTTP 伺服器，現在可以直接在瀏覽器版介面中透過 AJAX 呼叫完整的 MarkItDown 轉換能力：
...
1. 安裝並啟動 MCP 伺服器：
   ...
   ```
2. 開啟本網站後，在「連線本機 MarkItDown MCP 伺服器」區塊中勾選「使用本機 MCP 伺服器進行轉換」。
3. 如有需要，可調整伺服器位址並點擊「測試連線」確認狀態（請確保請求會指向 `/mcp/`，避免遭到 307 導向）。
4. 完成設定後，所有上傳文件皆會以資料 URI 方式透過 `convert_to_markdown` 工具送往 MCP 伺服器轉換，並即時回傳 Markdown 結果。

> ⚠️ 提醒：MCP 伺服器預設沒有身份驗證，請務必綁定於 `localhost` 或可信任的網路環境，避免將伺服器暴露在公共網路上。

若瀏覽器與 MCP 伺服器不在同一來源，可使用範例代理：

- Node.js 版本 (`website/server.js`)
  ```bash
  cd website
  npm install express http-proxy-middleware
  node server.js
  ```
- FastAPI 版本 (`website/server.py`)
  ```bash
  cd website
...
啟動後，在瀏覽器造訪 `http://127.0.0.1:8080` 並勾選使用 MCP，即可避開預檢 `OPTIONS` 造成的 405。
